#!/usr/bin/python
# Small script to install Spark 1.0.2 
import os 
import subprocess
import glob
import sys
...
#
scala_url = "s3://support.elasticmapreduce/spark/ami-3.1.1/scala-2.10.3.tgz"
spark_url = "s3://support.elasticmapreduce/spark/ami-3.1.1/spark-1.0.2.tgz"
...
def download_and_uncompress_files():
	...
	subprocess.check_call(["/bin/tar", "zxvf" , tmp_dir+"/spark-1.0.2.tgz", "-C", hadoop_apps])
	subprocess.check_call(["/bin/tar", "zxvf" , tmp_dir+"/scala-2.10.3.tgz", "-C", hadoop_apps])
	subprocess.check_call(["/bin/ln","-s",hadoop_apps+"/spark-1.0.2",spark_home])
	# cleanup 
	os.remove(tmp_dir+"/scala-2.10.3.tgz")
	os.remove(tmp_dir+"/spark-1.0.2.tgz")
...
def prepare_classpath():
	...
	subprocess.check_call(["/bin/mkdir",spark_classpath])
	subprocess.check_call(["/bin/cp","-R","/usr/share/aws/emr/emrfs/lib/",emr_fs])
...
def config():
	...
	if len(files) < 1:
		...
		lzo_jar=files[0]
	#subprocess.check_call(["/bin/mkdir",spark_log_dir])
	subprocess.call(["/bin/mkdir",spark_log_dir])
